...
$con = mysql_connect("localhost","root","") or die(mysql_error()); // connect to the db
mysql_select_db("medicalcenterdb",$con) or die(mysql_error()); // selects the right db
	if($_GET[checkNum]){ // if your load with ?checkNum=1 you just want to check if there is anything new (this is for optimization)
		$q = mysql_query("select count(*) as nb from treatment where InvestigationStatus = 0") or die(mysql_error());
		...
		$q = mysql_query("select * from treatment where InvestigationStatus = 0 order by TreatmentID limit 1") or die(mysql_error());
		$r = mysql_fetch_array($q);
		mysql_query("update treatment set InvestigationStatus = 1 where TreatmentID = $r[TreatmentID]");
		...
		echo "<a href ='showInvestigation.php?id=".$r[TreatmentID]."' target='iframe_mlt' >click </a>";
	}
?>
