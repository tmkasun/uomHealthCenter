...
function insertMessage($regNo, $diagnoses, $complaints, $treatments, $otherNotes, $refDoctor, $otherNotes, $link) { // #new add new argument $link
	...
	/*
	 * if($investigation == "None"){ $_investigationStatus=1; }else { $_investigationStatus=0; $query1= "INSERT INTO investigation_ok (StudentID,Investigation_okId,InvestigationName,isSubmit,TreatmentID,RefDoctor) VALUES('".$regNo."','','".$investigation."','','".$cur_auto_id."','".$_SESSION['USERNAME']."')"; $sql1=mysql_query("SELECT MAX(TreatmentID) FROM treatment",$link); $result0 = mysql_fetch_array($sql1); $cur_auto_id = $result0['MAX(TreatmentID)'] + 1; $result=mysql_query($query1,$link); if(!$result) die("Error: ".mysql_error()); }
	 */
}
	...
	switch ($_POST ['action']) {
		case "update" :
			...
			while ( $row = mysql_fetch_array ( $res ) ) {
				...
				//date
				?>
<font style="cursor: pointer;" id="<?php print $row["time_stamp"]?>"
	onclick='preTreatmentDetails(this)'><img src="../../images/bullet.gif"
	alt="-" /> <!-- Use  strtotime to display only year month date --> <?php print date("Y-F-d~g:i A", strtotime($row['time_stamp'])); ?>
<br /> </font>
<?php
